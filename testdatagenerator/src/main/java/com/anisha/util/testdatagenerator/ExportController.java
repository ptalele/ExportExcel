...
public class ExportController {
	...
	public List<String> mappings() {
		return new ArrayList(Arrays.asList(new String[] { "/generate/fn", "/generate/ln", "/generate/mn",
				"/generate/dob", "/generate/zip", "/generate/email", "/generate/master" }));
	}

	@GetMapping(value = "/generate/{field}")
	public ModelAndView exportRevisionsToExcel(final ModelAndView modelAndView, @PathVariable String field,
			final HttpServletResponse response) {
		try {

			// Read the Input XLS
			long startTime = System.currentTimeMillis();
			final ExcelModel model = xlsReader.readExcelData(
					"C:\\Users\\skmyne6m\\git\\ExportExcel\\testdatagenerator\\src\\main\\resources\\historypharmacyinput-master.xlsx"); // Generate
																																			// Data
																																			// Here
			...
			case "wph":
				DataGenerationUtil.updateCorelationId(model);
				DataGenerationUtil.updateMessgeId(model);
				DataGenerationUtil.updateWorkPhone(model);
				break;
			case "hph":
				DataGenerationUtil.updateCorelationId(model);
				DataGenerationUtil.updateMessgeId(model);
				DataGenerationUtil.updateHomePhone(model);
				break;
			case "stl1":
				DataGenerationUtil.updateCorelationId(model);
				DataGenerationUtil.updateMessgeId(model);
			...
			case "master":
				...
				DataGenerationUtil.updateWorkPhone(model);
				DataGenerationUtil.updateHomePhone(model);
				DataGenerationUtil.updateCellPhone(model);
				DataGenerationUtil.updateEmail(model);
				// DataGenerationUtil.updateCustStreetLine1(model);
				// DataGenerationUtil.updateCustStreetLine2(model);
				break;
			default:
				System.out.println("Available Operations :" + "/generate/fn" + "/generate/ln" + "/generate/mn"
						+ "/generate/dob" + "/generate/zip" + "/generate/email" + "/generate/master");
			}

			elapsedTime = (System.currentTimeMillis() - startTime) / 11000;
			...
			// final String excelFileName = "Revisions_" +
